...
<div class="flex flex-wrap items-center gap-1 mb-2">
  ...
  {#each views as view (view.id)}
    {#if renaming === view.id}
      <div class="flex items-center gap-1">
        ...
        <button type="button" onclick={() => (renaming = null)} class="text-xs text-ink-soft">✕</button>
      </div>
    {:else if deleteConfirm === view.id}
      <div class="flex items-center gap-1 px-1.5 py-0.5 bg-red-50 border border-red-200 rounded-full text-xs">
        ...
        <button type="button" onclick={() => (deleteConfirm = null)} class="text-ink-soft">Nein</button>
      </div>
    {:else}
  ...
  {#if saving}
    <div class="flex items-center gap-1">
      ...
      <button type="button" onclick={() => (saving = false)} class="text-xs text-ink-soft"><X class="w-3 h-3" /></button>
    </div>
  {:else}
    <button
      type="button"
      onclick={() => (saving = true)}
      class="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border border-dashed border-line text-ink-soft hover:border-terracotta hover:text-terracotta transition-colors"
    >
